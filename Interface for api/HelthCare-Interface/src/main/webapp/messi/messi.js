    ...
    function Messi(data, options) {
        ...
        if (_this.options.buttons.length > 0) {
            ...
            for (var i = 0; i < _this.options.buttons.length; i++) {
                ...
                var btnid = (_this.options.buttons[i]['id']) ? _this.options.buttons[i]['id'] : '';
...
function alertOpen(message, type) {
        var title = "";
        if (type === 'error') {
            title = "Error";
        ...
        var dialog = new Messi(
                message,
                {
                    modal: true, title: title, titleClass: 'anim ' + type, buttons: [{id: 'alertclsbtn', label: 'Close', val: 'X'}]
                }
        );
